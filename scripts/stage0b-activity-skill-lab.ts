...
/**
 * Validate a plan entry's URL against the existing pure URL resolver.
 * NO network. NO browser. Pure function call.
 */
function preflight(entry: LabTestPlanEntry): PreflightResult {
  const notes: string[] = [];
  const resolved = resolveTravelLinkFromUrl(entry.url);
  if (!resolved) {
  ...
  if (resolved.page_type !== entry.expected_resolver_page_type) {
    notes.push(
      `expected page_type=${entry.expected_resolver_page_type} got ${resolved.page_type}`,
    );
  }
  if (resolved.execution_mode !== entry.expected_resolver_execution_mode) {
    notes.push(
      `expected execution_mode=${entry.expected_resolver_execution_mode} got ${resolved.execution_mode}`,
    );
  }
