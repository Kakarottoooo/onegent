...
} from "@/lib/stage0b-skill-runtime/harness-script";
import type {
  LabEvent,
  LabHardStopReason,
  LabTestPlanEntry,
...
export function parseBrowserHarnessPayload(stdout: string): BrowserHarnessPayload {
  const start = stdout.indexOf(STAGE0B_HARNESS_SENTINEL_START);
  const end = stdout.indexOf(STAGE0B_HARNESS_SENTINEL_END);
  ...
  const jsonText = stdout.slice(start + STAGE0B_HARNESS_SENTINEL_START.length, end).trim();
  try {
