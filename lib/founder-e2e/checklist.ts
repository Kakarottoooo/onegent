...
export type PathId = "quick" | "full" | "auto";

export const PATH_LABEL: Record<PathId, string> = {
  quick: "Quick path (10 min)",
  full: "Full path (60-90 min)",
  auto: "Automated runner",
};

// Run source — distinguishes runs created by a human walking the page from
// those generated by `scripts/run-founder-e2e.mjs`. Old saved files (before
// 2026-05-04) had no source field; parse defaults to "manual" for those.
export type RunSource = "manual" | "automated";
...
// Verdict tier the autonomous runner uses to decide exit code.
export type RunnerVerdict = "pass" | "needs_polish" | "fail";

export const RUNNER_VERDICT_LABEL: Record<RunnerVerdict, string> = {
  pass: "PASS",
  needs_polish: "NEEDS_POLISH",
  fail: "FAIL",
};

export const RUNNER_VERDICT_TONE: Record<RunnerVerdict, "success" | "warning" | "danger"> = {
  pass: "success",
  needs_polish: "warning",
  fail: "danger",
};
...
// Schema bumped to 2 on 2026-05-04 when runner-source / runnerMeta /
// runnerVerdict landed. Parser still accepts schemaVersion=1 payloads —
// they default source to "manual" and skip runner-only fields.
export const FOUNDER_E2E_SCHEMA_VERSION = 2;
export const FOUNDER_E2E_KIND = "founder-e2e-run";

export const FOUNDER_E2E_LEGACY_SCHEMA_VERSION = 1;

export interface RunnerMeta {
  /** Exact command (or argv) the runner invoked itself with. */
  command: string;
  /** Base URL the runner exercised (e.g. http://localhost:3000). */
  baseUrl: string;
  /** Browser engine + version (e.g. "chromium 1.58.2"). */
  browser?: string;
  /** Total wall-clock duration of the run in milliseconds. */
  durationMs?: number;
  /** Playwright runtime version, when available. */
  playwrightVersion?: string;
  /** Node.js version used to launch the runner. */
  nodeVersion?: string;
  /** Optional label so CI / local parallel runs don't collide. */
  label?: string;
}
...
export interface QaRun {
  ...
  noteAtEnd?: string;
  /** "manual" (founder clicked) vs "automated" (runner). Default manual. */
  source: RunSource;
  /** Populated when source = "automated". */
  runnerMeta?: RunnerMeta;
  /** Computed verdict tier for the autonomous runner. */
...
export function parseQaRun(raw: unknown): QaRun {
  ...
  if (
    ...
    throw new FounderE2eParseError(
      `unknown schemaVersion ${String(schemaVersion)}; expected ${FOUNDER_E2E_SCHEMA_VERSION} or ${FOUNDER_E2E_LEGACY_SCHEMA_VERSION}`,
  ...
  if (pathId !== "quick" && pathId !== "full" && pathId !== "auto") {
    throw new FounderE2eParseError(`pathId must be "quick", "full", or "auto"`, "bad_path_id");
  }
  ...
  const sourceRaw = optionalString(obj.source);
  const source: RunSource =
    sourceRaw === "automated" || sourceRaw === "manual" ? sourceRaw : "manual";

  let runnerMeta: RunnerMeta | undefined;
  if (obj.runnerMeta && typeof obj.runnerMeta === "object") {
    const m = obj.runnerMeta as Record<string, unknown>;
    const command = optionalString(m.command);
    const baseUrl = optionalString(m.baseUrl);
    if (command && baseUrl) {
      runnerMeta = {
        command,
        baseUrl,
        browser: optionalString(m.browser),
        durationMs: numberFrom(m.durationMs),
        playwrightVersion: optionalString(m.playwrightVersion),
        nodeVersion: optionalString(m.nodeVersion),
        label: optionalString(m.label),
      };
    }
  }

  const runnerVerdictRaw = optionalString(obj.runnerVerdict);
  let runnerVerdict: RunnerVerdict | undefined;
  if (
    runnerVerdictRaw === "pass" ||
    runnerVerdictRaw === "needs_polish" ||
    runnerVerdictRaw === "fail"
  ) {
    runnerVerdict = runnerVerdictRaw;
  }
...
export function buildEmptyRun(
  ...
  const source: RunSource = options.source ?? "manual";
  return {
    schemaVersion: FOUNDER_E2E_SCHEMA_VERSION,
    kind: FOUNDER_E2E_KIND,
    id,
...
export function recomputeRun(
  ...
  const summary = summarizeResults(pathDef, normalized);
  const exit = decideExit(pathDef, normalized, defs);
  return {
    ...run,
    schemaVersion: FOUNDER_E2E_SCHEMA_VERSION,
    ...
    summary,
    exit,
    runnerVerdict: deriveRunnerVerdict({ summary, exit, run, pathDef }),
    updatedAt: now(),
  };
}

/**
 * Pure verdict resolver. Pass = no fail/blocker rows; needs_polish = some
 * P1+ fails but no P0 outstanding; fail = at least one P0 (status fail or
 * blocker) outstanding.
 *
 * Returns undefined for manual runs that haven't been touched yet so the
 * verdict pill doesn't render prematurely. The runner always sets source =
 * "automated" so it always gets a verdict.
 */
export function deriveRunnerVerdict(args: {
  summary: QaRunSummary;
  exit: ExitVerdict;
  run: QaRun;
  pathDef: ChecklistPath;
}): RunnerVerdict | undefined {
  const { summary, exit, run } = args;
  if (run.source !== "automated") return undefined;
  if (summary.total === 0) return undefined;
  // All-pending: probes haven't been recorded yet — indeterminate, not a
  // passing or failing run. The runner always records every probe, so this
  // path only fires for placeholder runs.
  if (summary.pending === summary.total) return undefined;
  if (summary.failing === 0 && summary.pending === 0 && summary.skipped === 0) return "pass";
  if (exit.p0Count > 0) return "fail";
  if (summary.failing > 0) return "needs_polish";
  if (summary.pending > 0 || summary.skipped > 0) return "needs_polish";
  return "pass";
}

export function exitCodeForVerdict(verdict: RunnerVerdict | undefined): number {
