  ...
  it("every plan entry's URL classifies through the existing pure URL resolver as the entry expects", () => {
    ...
    for (const entry of STAGE0B_TEST_PLAN) {
      ...
      if (resolved.page_type !== entry.expected_resolver_page_type) {
        failures.push(
          `${entry.id}: expected page_type=${entry.expected_resolver_page_type} got ${resolved.page_type}`,
        );
      }
      if (resolved.execution_mode !== entry.expected_resolver_execution_mode) {
        failures.push(
          `${entry.id}: expected execution_mode=${entry.expected_resolver_execution_mode} got ${resolved.execution_mode}`,
        );
      }
    }
