...
function contact(opts: Partial<ContactMatchInput> & { user_id: string }): ContactMatchInput {
  return {
...
describe("matchContactsFuzzy", () => {
  it("returns [] for empty input", () => {
    ...
    expect(matchContactsFuzzy([contact({ user_id: "u1" })], [])).toEqual([]);
  });

  it("Tier 1: exact display_name match (case-insensitive)", () => {
    const contacts = [
      contact({ user_id: "u1", display_name: "ZiweiB", username: "ziwei_b", profile_code: "@ziwei_b" }),
      contact({ user_id: "u2", display_name: "ZiweiC", username: "ziwei_c", profile_code: "@ziwei_c" }),
    ];
  ...
  it("Tier 1: exact match strips @ from profile_code on both sides", () => {
    const contacts = [
      contact({ user_id: "u1", profile_code: "@ziwei_b", display_name: "ZiweiB" }),
    ];
  ...
  it("Tier 1: nickname wins over noisy profile_code", () => {
    const contacts = [
      contact({ user_id: "u1", nickname: "李明", profile_code: "@xyz123" }),
    ];
  ...
  it("under-3-char tokens never trigger fuzzy fallback", () => {
    const contacts = [
      contact({ user_id: "u1", display_name: "ZiweiB" }),
      contact({ user_id: "u2", display_name: "ZiweiC" }),
    ];
  ...
  it("Tier 2: profile_code substring resolves uniquely", () => {
    const contacts = [
      contact({ user_id: "u1", profile_code: "@ziwei_b_main", display_name: "ZiweiB" }),
      contact({ user_id: "u2", profile_code: "@guo_wei", display_name: "Guo Wei" }),
    ];
  ...
  it("Tier 3: nickname/display_name prefix resolves uniquely", () => {
    const contacts = [
      contact({ user_id: "u1", display_name: "Zachary" }),
      contact({ user_id: "u2", display_name: "Bob" }),
    ];
  ...
  it("Tier 4: substring fallback works for Chinese characters", () => {
    const contacts = [
      contact({ user_id: "u1", display_name: "李明" }),
      contact({ user_id: "u2", display_name: "张三" }),
    ];
  ...
  it("blocks (returns candidates) when fuzzy hits multiple", () => {
    const contacts = [
      contact({ user_id: "u1", display_name: "ZiweiB", profile_code: "@ziwei_b" }),
      contact({ user_id: "u2", display_name: "ZiweiC", profile_code: "@ziwei_c" }),
    ];
    const out = matchContactsFuzzy(contacts, ["ziwei"]);
    expect(out[0].contact_user_id).toBeNull();
    expect(out[0].matched).toBeNull();
  ...
  it("blocks with empty candidates when no match at all", () => {
    const contacts = [
      contact({ user_id: "u1", display_name: "Alice" }),
    ];
  ...
  it("collapses _ - and whitespace consistently on both sides", () => {
    const contacts = [
      contact({ user_id: "u1", profile_code: "@ziwei_b" }),
    ];
