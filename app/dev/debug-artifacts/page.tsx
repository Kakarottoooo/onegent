...
function EmptyState() {
  return (
    <section className="dbga__empty">
      ...
      <p>
        Provider runs that hit a terminal guest-form failure write to{" "}
        <code>worker/.debug-screenshots/&lt;provider&gt;/</code>. None exist in
        this worktree right now. To populate:
      </p>
      <ul>
        <li>
          <strong>OpenTable</strong>: trigger an OpenTable booking that fails at
          phone-gate / guest-form — codex&apos;s provider writes to{" "}
          <code>opentable/&lt;ts&gt;-&lt;label&gt;/</code>.
        </li>
        <li>
          <strong>Resy</strong>: codex&apos;s recent <code>fix(resy)</code>{" "}
          patches add the same capture pattern; first failed Resy run lands
          here under <code>resy/</code>.
        </li>
      </ul>
      <p>
        If you expected runs here but see nothing, check that the dev server is
        running from the same worktree where the worker wrote the artifacts.
      </p>
    </section>
  );
}
