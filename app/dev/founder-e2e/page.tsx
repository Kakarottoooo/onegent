    ...
    async (file: string) => {
      try {
        const res = await fetch(
          `/api/dev/founder-e2e-runs?file=${encodeURIComponent(file)}`,
          { cache: "no-store" },
        );
        if (!res.ok) {
          setSaveState({
            status: "error",
            message: `Could not load ${file} (HTTP ${res.status})`,
          });
          return;
        }
        const json = (await res.json()) as { run?: QaRun };
        if (!json?.run) {
          setSaveState({ status: "error", message: "API response missing run" });
          return;
        }
        const loaded = json.run;
        // Switch path before swapping run so derived path/exit defs match.
        if (loaded.pathId !== pathId) {
          setPathId(loaded.pathId);
          // setRun runs after the path switch effect; queue via microtask.
          queueMicrotask(() => setRun(loaded));
        } else {
          setRun(loaded);
        }
        setActiveStepId(null);
        setSaveState({ status: "ok", file });
      } catch (err) {
        setSaveState({
          status: "error",
          message: err instanceof Error ? err.message : "Could not load run.",
        });
      }
    },
    [pathId],
    ...
    <div className="qa-page">
      ...
      <section className="qa-toprow">
        <fieldset className="qa-pathpicker" aria-label="Path picker">
          <legend>Path</legend>
          {(["quick", "full", "auto"] as const).map((id) => (
              ...
              {savedRuns.map((s) => {
                ...
                return (
                  <tr key={s.file}>
                    ...
                    <td>
                      <span
                        className={`qa-source-tag is-${s.source}`}
                        title={s.source === "automated" ? "Generated by scripts/run-founder-e2e.ts" : "Walked manually in this page"}
                      >
                        {RUN_SOURCE_LABEL[s.source]}
                      </span>
                    </td>
                    <td>
                      {verdict ? (
                        <span className={`qa-verdict-tag is-${RUNNER_VERDICT_TONE[verdict]}`}>
                          {RUNNER_VERDICT_LABEL[verdict]}
                        </span>
                      ) : s.meetsBar ? (
                        "✅"
                      ) : (
                    ...
                    <td>{s.baseUrl ?? "—"}</td>
                    <td>
                      {s.command ? <code className="qa-cmd">{truncate(s.command, 60)}</code> : "—"}
                    </td>
                    <td>
                      <button
                        type="button"
                        className="qa-btn qa-btn-ghost qa-btn-tiny"
                        onClick={() => void loadSavedRun(s.file)}
                      >
                        Open
                      </button>
                      <a
                        className="qa-saved-raw"
                        href={`/api/dev/founder-e2e-runs?file=${encodeURIComponent(s.file)}`}
                      >
                        raw
                      </a>
                    </td>
      ...
      {/* Automated-run preview (screenshots + command) for any run that has artifacts. */}
      <AutomatedRunSpotlight pathDef={pathDef} run={run} />
...
function AutomatedRunSpotlight({
  pathDef,
  run,
}: {
  ...
  if (run.source !== "automated") return null;
  const meta = run.runnerMeta;
  const screenshots = listAllSteps(pathDef)
    .map((step) => ({ step, result: run.results[step.id] }))
    .filter(({ result }) => result?.screenshotPath && isSafeRunnerAssetPath(result.screenshotPath));
  return (
    <section className="qa-spotlight">
      <header>
        <h2>Automated runner spotlight</h2>
        {run.runnerVerdict ? (
          <span className={`qa-verdict-tag is-${RUNNER_VERDICT_TONE[run.runnerVerdict]}`}>
            {RUNNER_VERDICT_LABEL[run.runnerVerdict]}
          </span>
        ) : null}
      </header>
      <dl className="qa-spotlight-meta">
        <div>
          <dt>Run id</dt>
          <dd>
            <code>{run.id}</code>
        ...
        <div>
          <dt>Node</dt>
          <dd>{meta?.nodeVersion ?? "—"}</dd>
        </div>
        <div>
          <dt>Label</dt>
          <dd>{meta?.label ?? "—"}</dd>
        </div>
        <div className="qa-spotlight-cmd">
          <dt>Command</dt>
          <dd>
      ...
      {screenshots.length ? (
        <div className="qa-spotlight-shots">
          <h3>Failure screenshots ({screenshots.length})</h3>
          <ul>
            {screenshots.map(({ step, result }) => (
              <li key={step.id}>
                <code>{step.id}</code> — {step.title} ·{" "}
                <a href={`/${result?.screenshotPath}`} target="_blank" rel="noreferrer">
                  open
                </a>
                <div className="qa-spotlight-actual">{result?.actual}</div>
              </li>
            ))}
          </ul>
        </div>
...
function FounderE2eStyles() {
  return (
    <style jsx global>{`
      ...
      .qa-source-tag.is-manual {
        background: #e5e7eb;
        color: #1f2937;
      }
      .qa-source-tag.is-automated {
        background: #dbeafe;
        color: #1e40af;
      }
      .qa-verdict-tag.is-success {
        background: #dcfce7;
        color: #166534;
      }
      .qa-verdict-tag.is-warning {
        background: #fef3c7;
        color: #92400e;
      }
      .qa-verdict-tag.is-danger {
        background: #fee2e2;
        color: var(--qa-fail);
      }
      .qa-cmd {
      ...
      .qa-spotlight-meta > div {
        display: flex;
        flex-direction: column;
      }
      .qa-spotlight-meta dt {
        color: var(--qa-muted);
        font-size: 10px;
        text-transform: uppercase;
        letter-spacing: 0.05em;
      }
      .qa-spotlight-meta dd {
        margin: 0;
        font-weight: 600;
      ...
      .qa-spotlight-shots ul {
        list-style: none;
        margin: 0;
        padding: 0;
        font-size: 12px;
      }
      .qa-spotlight-shots li {
        margin-bottom: 8px;
      }
      .qa-spotlight-actual {
        color: var(--qa-muted);
